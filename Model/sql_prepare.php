...
$sql_get_gallery_page = $db->prepare('
	SELECT a.`uuid`, a.`like_count`, a.`comment_count`, a.`created`, b.username as username
	FROM `uploads` as a JOIN `users` as b ON a.`user` = b.`uuid`
	ORDER BY a.`created` DESC
	LIMIT :start, :page_size
');
$sql_get_upload_comments = $db->prepare('
	SELECT b.username as username, a.`text`, a.`created`, a.`updated`
	FROM `comments` as a JOIN `users` as b ON a.`user` = b.`uuid`
	WHERE upload = :upload
');
$sql_get_login = $db->prepare('
	SELECT `uuid`, `email`, `username`, `email_on_comment`
	FROM `users`
	WHERE email = :email
	AND password = :pass
');
...
$sql_post_upload = $db->prepare('
	INSERT INTO `uploads`(`uuid`, `user`)
	VALUES (UUID(), :user)
');
$sql_post_comment = $db->prepare('
	INSERT INTO `comments`(`upload`, `user`, `text`)
	VALUES (:upload, :user, :text)
');
$sql_post_like = $db->prepare('
	INSERT INTO `likes`(`upload`, `user`)
	VALUES (:upload, :user)
');
